		...
		if self._query and self._connectstr:
			...
			if not self._newdata:
				...
				while True:
					...
					campos = OrderedDict()
					campos_tipo = OrderedDict()
					# pprint.pprint(self._cur.description)
					for i, coldesc in enumerate(self._cur.description, 1):
						if coldesc[0] in campos:
							campo = "%s_%d" % (coldesc[0], i)
						else:
							campo = coldesc[0]

						# campos[campo] = campo
						campos[campo] = coldesc[0]
						campos_tipo[campo] = coldesc[1]
						self._header.append(campo)

					d = {"rows": rows, "colnames": campos, "coltypes": campos_tipo}
					# pprint.pprint(d)
